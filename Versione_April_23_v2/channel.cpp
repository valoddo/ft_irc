...
/*   Updated: 2026/04/23 18:01:23 by vloddo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Channel::processJoin(Client& client, const std::string& pass, const std::string& server_name) //con void processJoin(Client& client, const std::string& password = "") di default se il secondo parametro non esiste, viene impostato a vuoto
{
    if(pass!= password){
    ...
    for (size_t i = 0; it != clients.end(); ++i, ++it)
...
*/
//482 ERR_CHANOPRIVSNEEDED - RIMOSSA PER ADESIONE AL CODICE CHE NON HA SINGOLE FUNZIONI DI ERRORE
...
//>> @time=2026-04-20T16:19:47.788Z :cacorreaa!~carolina@185.30.66.235 MODE #my42irc +i
				std::string	msg = ":" + client.getPrefix() + " MODE " + channel_name + 
					" " + flag;//manca il time all'inizio solo nel sender
        		broadcast(msg, NULL);
			}	
		}
...
//>> @time=2026-04-20T16:22:57.933Z :cacorreaa!~carolina@185.30.66.235 MODE #my42irc +k pwpw
					std::string flags = " +k ";
					std::string	msg = ":" + client.getPrefix() + " MODE " + channel_name + 
						flags + param[index];//manca time ini sender
					broadcast(msg, NULL);
				}
